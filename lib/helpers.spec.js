	...
	describe ('pipes', function () {
		...
		describe ('not_null()', function () {
			it ("can be used as a pipe before another helper, to filter out nulls and turn them into undefined", function () {
				let mapper = new GraphMapper({
					to: {
						normal_target: pipe('a_source', defaults_to('some default value')),
						not_null_target: pipe('a_source', not_null(), defaults_to('some default value')),
					},
				})
				assert.deepStrictEqual( mapper.read({ a_source: null}), { not_null_target: 'some default value', normal_target: null })
				assert.deepStrictEqual( mapper.read({ a_source: undefined}), { not_null_target: 'some default value', normal_target: 'some default value' })
			})
		})
