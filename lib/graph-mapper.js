...
class PropertyMapper  {
	...
	write(target, source) {
		to_array(
			this.write_pipe(_.get(target, this.target_path))
		)
		.forEach((value, i) => {
...
function read_pipe(fn_list, values) {
	return fn_list.slice(1).reduce((output, fn) => {
		return fn(output)
	}, fn_list[0](...values))
}


function write_pipe(fn_list, value) {
	return fn_list.reduce((output, fn) => {
		return fn(output)
	}, value)
}
