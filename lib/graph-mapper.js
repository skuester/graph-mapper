...
class PropertyMapper  {
	constructor(target_path, config, root_source_path) {
		...
		this.source_paths = config.from
		this.read_value = config.read || pass_thru
		this.write_value = config.write || pass_thru
	...
	read(source, target) {
		let values = this.source_paths.map(function (p) {
			return _.get(source, p)
		})

		let read_value = this.read_value(...values)

		this._assign(target, this.target_path, read_value)
	}


	write(target, source) {
		to_array(
			this.write_value(_.get(target, this.target_path))
		)
		.forEach((value, i) => {
	...
	_normalize_user_config(config) {
		...
		config.from = to_array(config.from)
