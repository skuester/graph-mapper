...
describe ("GraphMapper", function () {
	...
	describe ("ERRATA", function () {
		...
		it("should throw a helpful error message when mapper.use() is called under a 'to' property", function () {
			...
			assert.throws(() => {
			 ...
			 }, `don't use the 'to' property with mapper.use(). Correct is: my_output: mapper.use(...)`)
		})
	...
	describe ("helpers", function () {
		function test(path_config, input_value, output_value) {
			let mapper = new GraphMapper({ to: { target: path_config } })
			assert.deepEqual( mapper.read({source: input_value}).target ,  output_value )
		}

		describe (".strip_html()", function () {

			it ("removes html tags on read", function () {
				test( helpers.strip_html('source'), 'my <blink>Group Title</blink>', 'my Group Title' )
			})

			it ("only removes full tags", function () {
				test( helpers.strip_html('source'), 'my <blink>Group Title</blink', 'my Group Title</blink' )
			})

			it ("returns null if there is nothing left", function () {
				test( helpers.strip_html('source'), '<blink></blink>', null )
			})
		...
		describe (".not_null modifier", function () {
			it ("will translate a null source value into undefined before passing it on to the read function", function () {
				// This is helpful when the default value() will not overwrite a null source value
				test( helpers.value('source', 'default'), undefined, 'default' )
				test( helpers.value('source', 'default'), null, null )

				test( helpers.not_null.value('source', 'default'), undefined, 'default' )
				test( helpers.not_null.value('source', 'default'), null, 'default' )

				test( helpers.not_null.value('source'), null, undefined)
			})
		})
	})
